...
dat <- rbind.fill(dat_ou.raw, dat_ondrzk.raw);
...
```{r}
  pander(userfriendlyscience::descr(dat$age));
  userfriendlyscience::powerHist(dat$age);
```

## Sex

```{r}
  pander(userfriendlyscience::descr(factor(dat$sex)));
  userfriendlyscience::ggEasyBar(dat, 'sex');
```

## Education

```{r}
  pander(userfriendlyscience::descr(factor(dat$education)));
  userfriendlyscience::ggEasyBar(dat, 'education');
```

## Country

```{r}
  pander(userfriendlyscience::descr(dat$country));
```
...
```{r}
  pander(userfriendlyscience::descr(dat$urbanisation));
```

## Duration

We recorded how long participants took to complete the questionnaire.

```{r}
  pander(userfriendlyscience::descr(dat$Totaaltijd/60));
```
